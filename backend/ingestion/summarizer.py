...
- employee_count_consolidated は「連結従業員数」または「(2)連結の状況」の表の値を使用する。セクション名は「従業員の状況」「企業の概況」「提出会社の状況」のいずれかに含まれる場合がある
- 連結の記載が存在しない場合（単体のみ記載の会社）は「(1)提出会社の状況」または「提出会社の状況」の従業員数（正社員）を employee_count_consolidated に使用する
- employee_count_standalone は「(1)提出会社の状況」または「提出会社の状況」の従業員数（正社員）を使用する
- 従業員数の括弧内の数値（例: 679(147) の147）は臨時雇用者数であり、employee_count には含めない
- female_manager_ratio は持株会社等でグループ全体の数値がない場合、主要な子会社（銀行・証券等）の数値を使用する
- average_salary は万円単位で返す。表のヘッダーに記載された単位を読んで変換すること（百万円 → ×100、千円 → ÷10、円 → ÷10000、万円 → そのまま。例: 6.1百万円 → 610、895万円 → 895、8950千円 → 895）
